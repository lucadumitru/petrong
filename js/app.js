(() => {
    ...
    function initSliders() {
        if (document.querySelector(".slider-testimonials ")) {
            new core(".slider-testimonials ", {
                ...
                spaceBetween: 15,
                navigation: {
                    prevEl: ".slider-testimonials__button_prev",
                    nextEl: ".slider-testimonials__button_next"
                },
                breakpoints: {
                    0: {
                        slidesPerView: 1.1,
    ...
    let serviceItems = document.querySelectorAll(".item-services");
    document.querySelectorAll(".item-promise");
    document.querySelectorAll(".item-icon-list");
    let aplicationsImages = document.querySelectorAll(".aplications__image-ibg");
    for (let i = 0; i < serviceItems.length; i++) {
        fadeIn(serviceItems[i], 500 * i);
        function fadeIn(item, delay) {
            setTimeout((() => {
                item.classList.add("_fade-in");
            }), delay);
        }
    }
    for (let i = 0; i < aplicationsImages.length; i++) {
        fadeIn(aplicationsImages[i], 500 * i);
        function fadeIn(item, delay) {
            setTimeout((() => {
                item.classList.add("_fade-in");
            }), delay);
    ...
    const btnScrollToTop = document.querySelector(".fixed-elements__button_scroll");
    if (btnScrollToTop) window.addEventListener("scroll", (() => {
        const scrollPosition = window.scrollY;
        const scrollHeight = document.documentElement.scrollHeight;
        const windowHeight = window.innerHeight;
        const scrollPercentage = scrollPosition / (scrollHeight - windowHeight) * 100;
        if (scrollPercentage >= 80) btnScrollToTop.classList.add("_active"); else btnScrollToTop.classList.remove("_active");
    }));
    const items = document.querySelectorAll(".item-icon-list");
    const resizeObserver = new ResizeObserver((entries => {
        entries.forEach((entry => {
            const content = entry.target.querySelector(".item-icon-list__content");
            const line = entry.target.querySelector(".item-icon-list__icon span");
            const height = content.offsetHeight / 2;
            const nextSibling = entry.target.nextElementSibling;
            if (nextSibling) {
                const nextContent = nextSibling.querySelector(".item-icon-list__content");
                const nextHeight = nextContent.offsetHeight / 2 + 10;
                line.style.height = `${(height + nextHeight) / 16}rem`;
            }
        }));
    }));
    items.forEach((item => {
    ...
    const script_anchor = document.querySelector(".chat-window__anchor");
    const chatBody = document.querySelector(".chat-window__body");
    const chatButton = document.querySelector(".fixed-elements__button_chat");
    const chatWindow = document.querySelector(".chat-window");
    const chatButtonClose = document.querySelector(".header-chat__button-close");
    if (chatWindow) {
        ...
        function sendMessage() {
            ...
            messageElement.classList.add("chat-window__user-message");
            messageElement.innerText = messageText;
            chatBody.insertBefore(messageElement, script_anchor);
            userInput.value = "";
            console.log(userInput.value);
            chatBody.scrollTop = chatBody.scrollHeight;
        }
        chatBody.addEventListener("scroll", (function() {
            if (chatBody.scrollTop < chatBody.scrollHeight - chatBody.clientHeight) chatBody.classList.add("chat-window__body--scrolled"); else chatBody.classList.remove("chat-window__body--scrolled");
        }));
        chatButton.addEventListener("click", openChat);
        function openChat() {
            chatWindow.classList.toggle("_hidden");
            setTimeout((function() {
                const existingMessages = document.querySelectorAll(".chat-window__user-message, .chat-window__bot-message");
                if (0 === existingMessages.length) {
                    const messageElement = document.createElement("div");
                    messageElement.classList.add("chat-window__bot-message");
                    messageElement.innerText = "How may we assist you today?";
                    chatBody.insertBefore(messageElement, script_anchor);
                    chatBody.scrollTop = chatBody.scrollHeight;
                }
            }), 1e3);
        }
        chatButtonClose.addEventListener("click", closeChat);
        function closeChat() {
            chatWindow.classList.toggle("_hidden");
        }
    }
    ...
    menuLinks.forEach((link => {
        if (link.href === currentUrl) link.closest(".menu__item").classList.add("_active");
    }));
    const svgLines = document.querySelectorAll(".svg path");
    svgLines.forEach((line => {
        console.log(line);
        let lineLength = line.getTotalLength();
        line.style.strokeDasharray = lineLength;
        line.style.strokeDashoffset = lineLength;
        line.classList.add("svg_animation");
    }));
    window["FLS"] = true;
    isWebp();
    menuInit();
